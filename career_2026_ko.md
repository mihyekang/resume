...
GPU 미확보라는 사업 제약을 아키텍처 결정으로 흡수했습니다. 자체 모델(QWEN) 운영이 불가능하다고 판단해 API 기반 Gemini Flash 단일 처리 구조로 전환했고, GPU 미가용의 원인이 기술이 아니라 사업부서의 예산 미확보와 물리 장비 부족임을 구분해 PO 의사결정 사항으로 문서화했습니다. 개발팀이 풀 수 없는 문제를 블로커로 남기지 않고 결정 권한이 있는 곳으로 이관하는 방식입니다.
...
- GPU 미확보 환경에서 자체 모델(QWEN) 운영 불가로 판단하고 API 기반 Gemini Flash 단일 처리 구조로 전환
...
Siebel CRM 운영과 Legacy 인터페이스 개발로 시작해 차세대 영업전산 전환의 모듈 리더를 맡으며 개발과 운영의 양쪽을 경험했습니다. 이후 IoT 운영이관과 빅데이터 포털 구축을 거치며 여러 조직이 얽힌 구조에서 산출물을 정렬하는 일을 담당했고, 챗봇 기간계 연동과 기가지니 백엔드 플랫폼에서 대고객 서비스의 기술 요구를 직접 다뤘습니다.

Azure 대규모 이관에서는 설계 기준이 없던 영역을 직접 정의하고 Kubernetes Manifest 작성까지 수행하며 엔지니어와 같은 언어로 소통하는 범위를 넓혔고, 현재 AI 미디어 에이전트 연동에서는 연동 규격과 아키텍처 수준의 의사결정을 조율하고 있습니다. 요구사항을 전달하는 관리자에서, 기술 제약과 사업 제약을 구분해 결정 권한이 있는 곳으로 이관하고 다음 사람이 쓸 기준을 남기는 방향으로 역할을 확장해 왔습니다.
